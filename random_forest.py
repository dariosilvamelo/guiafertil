...
class RandomForest():
    ...
    def create_regressor(self):
        seed = int(self.register.edt_seed_random_forest.text())
        self.set_seed(seed)
        ...
        self.regressor = RandomForestRegressor(n_estimators=int(self.register.edt_n_tree.text()),
                                               random_state=seed,
                                               bootstrap=True)
    ...
    def cross_validation(self):
        if self.validate_parameters():
            ...
            self.register.edt_process.append('Floresta Aleatória: ')
            self.register.edt_process.append(' ')
    ...
    def load_files(self):
        directory = './model/random_forest/regressors'
        if directory:
            self.register.list_random_forest.clear()
            try:
                for filename in os.listdir(directory):
                    self.register.list_random_forest.addItem(filename)
            except Exception as e:
                self.register.list_random_forest.addItem(f"Erro: {str(e)}")

    def validate_parameters(self):
        try:
            training = int(self.register.edt_training.text())
            n_tree = int(self.register.edt_n_tree.text())
            cv = int(self.register.edt_cv.text())
        except Exception as e:
            message('erro', 'O percentual de dados para treinamento, o n° de Folds e o nº de árvores devem ser preenchidos!' + str(e))
            return False
        return True 
